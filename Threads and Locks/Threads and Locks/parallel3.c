...
void *sum_array(void *arg){
    ...
    int localSum = 0; //sum within each pThread
	
    for (i=0; i < ARRAY_SIZE/THREAD_NO; i++){
	localSum += subArray[i];
    }

    //lock once all the calculations of localSum occur, then add to sum, then unlock afterwards	
...
int main(){
    ...
    //check mutex initialization failure
